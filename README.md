...
### 5. 구간(iterator)으로 벡터 다루기

`begin()`은 첫 원소의 위치, `end()`는 마지막 원소 "다음"의 위치를 가리킴 (즉 `end()`가 가리키는 자리엔 실제 값이 없음). 그래서 항상 시작은 포함, 끝은 미포함.

```cpp
vector<int> v = {1,2,3,4};
v.begin()   // 1의 위치
v.end()     // 4 바로 다음의 위치 (값 없음)
```

#### 구간으로 새 벡터 만들기
`vector<int> 변수명(시작 위치, 끝 위치);` 형태로, 기존 벡터의 특정 구간만 잘라 새 벡터를 만들 수 있음.
```cpp
